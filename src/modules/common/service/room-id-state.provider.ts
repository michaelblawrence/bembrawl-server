...
export class RoomIdStateProvider {
    ...
    public claimRoomId(): number {
        const randomFloat = Math.random() * this.availableRoomIds.size;
        const roomArrayIdx = Math.floor(randomFloat);
        const roomId = Array.from(this.availableRoomIds)[roomArrayIdx];
